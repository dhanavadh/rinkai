...
<header class={`w-full ${$stickyClass} top-0 z-50`}>
	<nav
		class={`relative hidden w-full items-center justify-between font-ibm transition-transform duration-300 ease-in-out md:flex ${$backgroundClass} ${$isScrollingDown ? '-translate-y-full' : 'translate-y-0'}`}
	...
	<nav
		class={`relative w-full items-center justify-between font-ibm md:hidden ${$backgroundClass}`}
	>
		<MobileNavbar />
	</nav>
</header>
